...
class ReluLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        derivative = np.where(self._cache_current > 0, 1.0, 0.0)
        return grad_z * derivative 
...
            - neurons {list} -- Number of neurons in each linear layer 
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        activationMap = {'sigmoid':lambda : SigmoidLayer(), 'relu':lambda : ReluLayer()}
        self._layers = [LinearLayer(input_dim, neurons[0])]
        if activations[0] in activationMap:
            self._layers.append(activationMap[activations[0]]())
        
        for i in range(len(neurons) - 1):
            self._layers.append(LinearLayer(neurons[i], neurons[i + 1]))
            if activations[i + 1] in activationMap:
                self._layers.append(activationMap[activations[i + 1]]())
...
class Trainer(object):
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        rand_order = np.random.permutation(len(input_dataset))
        return input_dataset[rand_order], target_dataset[rand_order]
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for _ in range(self.nb_epoch):
            input_dataset, target_dataset = input_dataset, target_dataset if self.shuffle_flag else self.shuffle(input_dataset, target_dataset)
            batch_size = self.batch_size
            for batch_start_idx in range(0, input_dataset.shape[0] - batch_size, batch_size):
                batch_end_idx =  batch_start_idx + batch_size
                training_batch = input_dataset[batch_start_idx: batch_end_idx]
                batch_pred_results = self.network(training_batch)
                batch_target = target_dataset[batch_start_idx: batch_end_idx]
                self._loss_layer(batch_pred_results, batch_target)
